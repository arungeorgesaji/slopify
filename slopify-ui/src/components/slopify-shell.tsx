...
export function SlopifyShell() {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const searchInput =
        window.innerWidth < 640 ? mobileSearchRef.current : desktopSearchRef.current
      ...
      <div className="min-h-svh bg-background">
        <header className="fixed inset-x-0 top-0 z-40 border-b border-primary/80 bg-primary text-primary-foreground shadow-lg shadow-primary/20">
          <div className="mx-auto flex h-20 w-full max-w-7xl items-center gap-4 px-4 sm:px-6 lg:px-8">
            <div className="min-w-0 flex-1 text-2xl font-bold tracking-tight sm:flex-none sm:text-3xl">
              Slopify
            </div>
        ...
        <main className="w-full px-4 pb-44 pt-28 sm:px-6 lg:px-8">
          <Outlet />
        </main>
